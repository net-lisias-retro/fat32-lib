...
public class MsxDos extends de.waldheinz.fs.fat.FatFileSystem {

	MsxDos(final BlockDevice device, final BootSector bs, final boolean readOnly) throws IOException {
    ...
    public static FatFileSystem read(final BlockDevice device, final boolean readOnly) throws IOException {
    	return new MsxDos(device, new MsxBootSector(device), readOnly);
    }
}